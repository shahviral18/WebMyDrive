...
class AdminController
{
    ...
    {
        $userCount = Database::count('"User"', 'role != \'SUPERADMIN\' AND (deletedAt IS NULL OR deletedAt = \'\')');
    ...
    public function getUsers(Request $req): void
    {
        ...
        foreach ($users as $u) {
            ...
            $wsPlanId     = isset($u['wsPlanId']) ? (int)$u['wsPlanId'] : null;
            $planMismatch = $latestOrder && $latestOrder['planId'] && $wsPlanId && (int)$latestOrder['planId'] !== $wsPlanId;
    ...
    public function createUser(Request $req): void
    {
        ...
        $defaultPass = ucfirst(strtolower($basePass)) . strtoupper(bin2hex(random_bytes(2)));
        ...
        if ($role === 'USER') {
            ...
            $referralCode = substr($baseRef, 0, 12) . strtoupper(bin2hex(random_bytes(2)));
        }
    ...
    public function createDistributor(Request $req): void
    {
        ...
        for ($attempt = 1; $attempt <= 10; $attempt++) {
            ...
            if (!$conflict)
                break;
            $referralCode = $baseCode . strtoupper(substr(dechex(time()), -4));
            if ($attempt === 10)
    ...
    {
        $id = (int) ($req->params['id'] ?? 0);
        if (!$id) Response::error('Distributor ID required', 400);

        $dist = Database::queryOne('SELECT id FROM "Distributor" WHERE id = :id', [':id' => $id]);
    ...
    public function deletePlan(Request $req): void
    {
        ...
        // Block deletion if any active workspace is on this plan
        $activeCount = Database::count('"Workspace"', 'planId = :id AND status = \'ACTIVE\'', [':id' => $id]);
        if ($activeCount > 0)
            Response::error("Cannot delete: $activeCount active workspace(s) are on this plan. Deactivate or reassign them first.", 409);
    ...
    private function sendPlanChangeEmails(
        ...
        $bodyUser =
            ...
            . ($note ? "Note          : {$note}\n" : '')
            . "\nIf you have any questions, please contact support@webmydrive.com\n\n"
            . "Thank you,\nWebMyDrive Team";
    ...
    public function importExistingUser(Request $req): void
    {
        ...
        $plainPassword = ucfirst(strtolower($basePass)) . strtoupper(bin2hex(random_bytes(2))) . '!';
        $passwordHash  = password_hash($plainPassword, PASSWORD_BCRYPT);
        ...
        while (Database::queryOne('SELECT id FROM `User` WHERE referralCode = :c', [':c' => $referralCode])) {
            $suffix++;
            $referralCode = substr($baseRef, 0, 10) . strtoupper(bin2hex(random_bytes(2))) . $suffix;
        }
        ...
        $userId = Database::insert(
            'INSERT INTO `User` (name, email, recoveryEmail, recoveryPhone, passwordHash, role, referralCode, walletBalance, passwordResetRequired, first_login, createdAt, updatedAt)
             VALUES (:name, :email, :recEmail, :recPhone, :hash, \'USER\', :code, 0, 1, 1, :createdAt, :updatedAt)',
            [
                ':name'      => $fullName,
                ':email'     => $email,
                ...
                ':hash'      => $passwordHash,
                ':code'      => $referralCode,
                ':createdAt' => $activationDate . ' 00:00:00',
                ':updatedAt' => $activationDate . ' 00:00:00',
            ]
        );
