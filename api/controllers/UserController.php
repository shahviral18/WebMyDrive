...
class UserController
{
    ...
    public function initiateUpgrade(Request $req): void
    {
        ...
        $zohoSessionId = $session['payments_session_id'] ?? ($session['id'] ?? '');
    ...
    public function confirmUpgrade(Request $req): void
    {
        ...
        $newRenewal      = $ws['renewalDate'] ?? null; // upgrade preserves existing renewal date
        $lockedDisc      = (float)($ws['discount_percent'] ?? 0);
    ...
    public function redeemVoucher(Request $req): void
    {
        ...
        $expiryMonths = (int) (ConfigService::getUserReferralConfig()['creditExpiryMonths'] ?? 24);
        $creditExpiry = date('Y-m-d H:i:s', strtotime("+{$expiryMonths} months"));
