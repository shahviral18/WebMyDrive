...
class PaymentController
{
    ...
    public function createSession(Request $req): void
    {
        ...
        // Wallet deduction — enforce configurable redemption cap (default 80%)
        // At least (100 - walletRedemptionMaxPct)% must always be paid via Zoho.
        $walletConfig         = ConfigService::getWalletConfig();
        $walletRedemptionMaxPct = (float) ($walletConfig['walletRedemptionMaxPct'] ?? 80);
        $maxWalletAllowed     = round($finalAmount * ($walletRedemptionMaxPct / 100), 2);
        $walletDeduction      = 0.0;
        $existingUserId       = null;
        ...
        if ($existingUserRow && $useWalletAmount > 0) {
            ...
            $walletDeduction = min($useWalletAmount, $availableWallet, $maxWalletAllowed);
            $existingUserId  = (int) $existingUserRow['id'];
        }
        $amountAfterWallet = round($finalAmount - $walletDeduction, 2);
        // Always charge at least (100 - walletRedemptionMaxPct)% via Zoho
        $zohoChargeAmount = max(round($finalAmount * (1 - $walletRedemptionMaxPct / 100), 2), $amountAfterWallet);
        ...
        Response::json([
            ...
            'walletRedemptionMaxPct' => $walletRedemptionMaxPct,
    ...
    private function handlePaymentSucceeded(array $payload): void
    {
        ...
        try {
            ...
            if ($existing) {
                ...
                $userId = Database::insert(
                    ...
                    [
                        ...
                        ':hash'         => $meta['passwordHash'] ?? password_hash(bin2hex(random_bytes(16)), PASSWORD_BCRYPT),
                        ':ref'          => $refCode,
            ...
            $orderId = Database::insert(
                'INSERT INTO `Order`
                 (userId, planId, amount, currency, status, paymentId, gatewayTxId, promoCode, createdAt, updatedAt)
                 VALUES (:uid, :planId, :amount, \'INR\', \'PAID\', :payId, :ref, :promo, :now1, :now2)',
                [
                    ':uid'    => $userId,
            ...
            try {
                ...
                // Normalise to E.164 +91XXXXXXXXXX: strip leading 91 or 0, then prepend +91
                if ($rawPhone) {
                    if (str_starts_with($rawPhone, '91') && strlen($rawPhone) === 12) {
                        $rawPhone = substr($rawPhone, 2); // strip leading 91
                    } elseif (str_starts_with($rawPhone, '0')) {
                        $rawPhone = ltrim($rawPhone, '0');
                    }
                }
                $recoveryPhone = ($rawPhone && strlen($rawPhone) === 10) ? '+91' . $rawPhone : null;
