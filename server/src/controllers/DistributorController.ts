...
export class DistributorController {
    ...
    static async onboard(req: Request, res: Response) {
        try {
            ...
            await prisma.user.update({ where: { id: user.id }, data: { distributorId: dist.id } });
    ...
    static async getDashboard(req: Request, res: Response) {
        try {
            ...
            const activeLink = await ReferralLinkService.getActiveLink(distributorId, 'DISTRIBUTOR');

            return res.json({
                distributor: dist,
                nextTier,
                // Expose referralCode so frontend can build the correct referral link
    ...
    static async simulateSale(req: Request, res: Response) {
        try {
            ...
            const order = await prisma.order.create({
                data: { userId: Number(purchasingUserId), amount: Number(amount), status: 'PAID' }
            });
    ...
    static async getPayouts(req: Request, res: Response) {
        try {
            ...
            return res.json(payouts.map((p: any) => ({
                ...
                amount: `INR ${Math.abs(p.amount).toLocaleString()}`,
    ...
    static async getCustomers(req: Request, res: Response) {
        try {
            ...
            const distributorId = Math.abs(reqUser.userId);
            const directCustomers = await prisma.user.findMany({
                where: {
                    distributorId,
                },
            ...
            const sales = await prisma.distributorSale.findMany({
                where: { distributorId },
                select: { purchasingUserId: true }
            });
            const saleUserIds = [...new Set(sales.map((s: any) => s.purchasingUserId))];

            const indirectCustomers = await prisma.user.findMany({
                where: {
                    id: { in: saleUserIds },
                    distributorId: { not: distributorId },
                },
                include: { workspaces: { include: { plan: true } } },
                orderBy: { createdAt: 'desc' }
            });

            const allCustomers = [...directCustomers, ...indirectCustomers];

            const mapped = await Promise.all(allCustomers.map(async (c: any) => {
                ...
                return {
                    ...
                    email: c.email,
                    phone: c.phone || '-',
                    plan: c.workspaces?.[0]?.plan?.name || 'Free/None',
                    status: c.workspaces?.[0]?.plan ? 'active' : 'trial',
                    joined: c.createdAt.toLocaleDateString(),
                    commission: `INR ${totalCommission._sum.commissionEarned || 0}`,
                    level: c.distributorId === distributorId ? 'Direct' : 'L2 Chain'
                };
            }));
