/**
 * Environment and API configuration
 * Adjust these URLs based on your deployment environment
 */

export const API_CONFIG = {
  // Admin Console Backend
  ADMIN_API_URL: (() => {
    const url = import.meta.env.VITE_ADMIN_API_URL || "";
    if (!import.meta.env.DEV && (url.includes("localhost") || url.includes("127.0.0.1"))) {
      return "";
    }
    return url;
  })(),

  CLOUD_PLAN_MANAGER_URL: import.meta.env.VITE_CLOUD_PLAN_MANAGER_URL || "https://test.webmydrive.com",

  // Redirect destination after successful plan purchase
  PURCHASE_SUCCESS_REDIRECT: import.meta.env.VITE_PURCHASE_SUCCESS_REDIRECT || "https://test.webmydrive.com/login",
};

export default API_CONFIG;
